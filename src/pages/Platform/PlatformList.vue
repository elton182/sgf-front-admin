      ...
      <div class="md-layout-item md-medium-size-100 md-xsmall-size-100 md-size-100">
        <md-card>
          <md-card-header data-background-color="green">
            <h4 class="title">Cadastro de Plataformas</h4>
          </md-card-header>
          <md-card-content>
            <md-button class="md-success" @click="addPlatform">Adicionar Plataforma</md-button>
              ...
              <md-table-row slot="md-table-row" slot-scope="{ item, index }">
                ...
                <md-table-cell md-label="Ações">
                  <md-button class="md-just-icon md-warning"
                    @click="editPlatform(item.id, index)"><md-icon>edit</md-icon></md-button>
                  <md-button class="md-just-icon md-danger"
                    @click="delPlatform(item.id, index)"><md-icon>delete</md-icon></md-button>
                </md-table-cell>
              </md-table-row>
...
export default {
  ...
  methods: {
    ...
    async addPlatform() {
      this.$router.push('/platforms/add');
    },
    async editPlatform(id, index) {
      this.$router.push(`/platforms/edit/${id}`);
    },
    async delPlatform(id, index) {
      try {
        await Platform.delPlatform(id);
    ...
    getList() {
      Platform.getList('search=' + this.search).then(response => {
