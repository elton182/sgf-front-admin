      ...
      <div class="md-layout-item md-medium-size-100 md-xsmall-size-100 md-size-100">
        <md-card>
          <md-card-header data-background-color="green">
            <h4 class="title">Cadastro de Marcas</h4>
          </md-card-header>
          <md-card-content>
            <md-button class="md-success" @click="addBrand">Adicionar Marca</md-button>
              ...
              <md-table-row slot="md-table-row" slot-scope="{ item, index }">
                ...
                <md-table-cell md-label="Ações">
                  <md-button class="md-just-icon md-warning"
                    @click="editBrand(item.id, index)"><md-icon>edit</md-icon></md-button>
                  <md-button class="md-just-icon md-danger"
                    @click="delBrand(item.id, index)"><md-icon>delete</md-icon></md-button>
                </md-table-cell>
              </md-table-row>
...
export default {
  ...
  methods: {
    ...
    async editBrand(id, index) {
      this.$router.push(`/brands/edit/${id}`);
    },
    async delBrand(id, index) {
      try {
        await Brand.delBrand(id);
    ...
    getList() {
      Brand.getList('search=' + this.search).then(response => {
